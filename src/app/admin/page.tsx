...
type AnyPost = (Post | StoredPost) & { source: "mock" | "user" };

/* ── 유저 목록 읽기 ── */
function loadUsers(): Array<{ email: string; name: string; memberType: string; createdAt: string }> {
  try {
...
function deleteUser(email: string) {
  try {
    ...
    if (session && JSON.parse(session).email === email) {
...
const typeLabel: Record<string, string> = {
  free: "자유", review: "후기", promotion: "홍보",
};
const memberLabel: Record<string, string> = {
  admin: "관리자", business: "업소회원", general: "일반회원",
};
...
export default function AdminPage() {
  ...
  const refresh = useCallback(() => {
    ...
    const mock: AnyPost[] = [
      ...freePosts.map(p => ({ ...p, source: "mock" as const })),
      ...reviewPosts.map(p => ({ ...p, source: "mock" as const })),
      ...promotionPosts.map(p => ({ ...p, source: "mock" as const })),
    ];
    const stored: AnyPost[] = userPosts.map(p => ({ ...p, source: "user" as const }));
  ...
  const confirmAndDelete = () => {
    ...
    if (confirmDelete.type === "post") {
      deletePost(Number(confirmDelete.id));
      ...
      deleteUser(String(confirmDelete.id));
    }
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              { label: "전체 댓글", value: comments.length, color: "bg-purple-50 text-purple-700" },
              { label: "전체 회원", value: users.length, color: "bg-orange-50 text-orange-700" },
            ].map(stat => (
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredPosts.map(p => (
                  <tr key={`${p.source}-${p.id}`} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-400 hidden md:table-cell">{p.createdAt}</td>
                    <td className="px-4 py-3 text-right">
                      {p.source === "user" ? (
                        ...
                        <span className="text-xs text-gray-300">기본 데이터</span>
                      )}
                    </td>
                  </tr>
                ))}
